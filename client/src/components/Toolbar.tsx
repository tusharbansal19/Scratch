...
export default function Toolbar() {
    ...
    return (
        <>
            ...
            <div className={`
                fixed z-40 flex flex-col gap-2 md:gap-4 transition-all duration-300 ease-in-out origin-bottom-left
            ...
            `}>
                {/* Main Toolbar */}
                <div className="bg-[#1e1e1e]/90 backdrop-blur-md p-2 md:p-3 rounded-xl md:rounded-2xl border border-gray-700 shadow-2xl flex flex-col gap-1 md:gap-3">
                    {tools.map((t) => (
                        <button
                            key={t.id}
                            onClick={() => {
                    ...
                    <div className="relative">
                        <button
                            ...
                            className="w-8 h-8 md:w-10 md:h-10 rounded-lg md:rounded-xl border-2 border-gray-600 flex items-center justify-center transition-transform hover:scale-105 mx-auto"
                            style={{ backgroundColor: color }}
                        >
                            <div className="w-full h-full rounded md:rounded-lg ring-1 ring-inset ring-black/20" />
                        </button>

                        {/* Popover Color Picker */}
                        {showColorPicker && (
                            <div className="absolute left-full ml-4 top-auto bottom-0 md:top-0 md:bottom-auto p-2 md:p-3 bg-[#1e1e1e] rounded-xl border border-gray-700 shadow-xl grid grid-cols-3 gap-1 md:gap-2 w-32 md:w-48 z-50">
                                {['#3b82f6', '#ef4444', '#10b981', '#f59e0b', '#8b5cf6', '#ec4899', '#ffffff', '#000000', '#9ca3af'].map((c) => (
                                    <button
                                        ...
                                        className="w-6 h-6 md:w-8 md:h-8 rounded-md md:rounded-lg border border-gray-600 hover:scale-110 transition-transform"
                ...
                {/* Background Controls */}
                <div className="bg-[#1e1e1e]/90 backdrop-blur-md p-1.5 md:p-2 rounded-xl md:rounded-2xl border border-gray-700 shadow-xl flex flex-col gap-1 md:gap-2">
                    {backgrounds.map((bg) => (
                        <button
                            key={bg.id}
                            onClick={() => {
                            ...
                            className={`p-1.5 md:p-2 rounded-lg transition-colors ${backgroundColor === bg.id ? 'bg-gray-700 text-white' : 'text-gray-500 hover:text-gray-300'
                                }`}
                            title={bg.label}
                        >
                            <bg.icon className="w-4 h-4 md:w-[18px] md:h-[18px]" />
                        </button>
                    ))}
                </div>
            </div>
